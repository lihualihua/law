...
//    public Set<Role> findByUsers_IdAndStatus(Long id, Byte status);
    public Set<Role> findByDepts_IdAndStatus(Long id, Byte status);
     ...
     * @param status 角色状态
     */
    public Boolean existsByDepts_IdAndStatus(Long id, Byte status);
    ...
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM timo_sys_user_role WHERE role_id in ?1", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM timo_sys_role_menu WHERE role_id in ?1", nativeQuery = true)
